...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="filenameLeft_structure"
        select="concat('../SVG/',  $fileref[1], '/', $fileref[1], '_', 'leftEndleaves_structure', '.svg')"/>
    <xsl:variable name="filenameLeft_use"
        select="concat('../SVG/',  $fileref[1], '/', $fileref[1], '_', 'leftEndleaves_use', '.svg')"/>
    <xsl:variable name="filenameRight_structure"
        select="concat('../SVG/',  $fileref[1], '/', $fileref[1], '_', 'rightEndleaves_structure', '.svg')"/>
    <xsl:variable name="filenameRight_use"
        select="concat('../SVG/',  $fileref[1], '/', $fileref[1], '_', 'rightEndleaves_use', '.svg')"/>
    ...
    <xsl:template name="leftEndleaves">
        ...
        <xsl:choose>
            ...
            <xsl:when test="self::node()[yes | no]">
                ...
                <!--<xsl:for-each select="./yes/type[integral]">
                    <xsl:call-template name="leftEndleavesIntegral">
                        <xsl:with-param name="use" select="$use"/>
                    </xsl:call-template>
                </xsl:for-each>
                <xsl:for-each select="./yes/type[separate]">
    ...
    <xsl:template name="leftEndleavesSeparate">
        ...
        <xsl:for-each select="./yes/type/separate/units/unit">
            ...
            <xsl:call-template name="leftEndleavesSeparate_components">
                ...
                <!-- NB: 20130710 modification from: if (following-sibling::unit[1]/components/component//type[textHook]) then $Ay - $delta * 1.5
                    else $Ay - ((if (ancestor::type/preceding-sibling::type[integral]) then $delta * ancestor::type/preceding-sibling::type/integral/xs:integer(numberOfLeaves) else 0) 
                    ...
                    - (if (components/component//type[textHook]) then 0 else $delta * 2 * count(following-sibling::unit/components/component) 
    ...
    <xsl:template name="leftEndleavesSeparatePastedown-Guard">
        ...
        <g xmlns="http://www.w3.org/2000/svg">
            ...
            <path>
                ...
                <!-- Because the schema does not go down to the element level and does not allow for each part of the guard 
                    to be pasted as pastedown, this stub, if it has no following siblings, is drawn as uncertain to accommodate for the possibility that it 
                    was actually a pastedown -->
                <xsl:call-template name="certainty">
                    <xsl:with-param name="certainty"
    ...
    <xsl:template name="leftEndleavesSeparateFlyleaves-TextHook">
        ...
        <xsl:param name="currentComponent"/>
        <xsl:param name="certainty" select="100"/>
        <!--<xsl:variable name="integral"
            select="if (ancestor::yes/type/integral) then ancestor::yes/type/integral/numberOfLeaves else 0"/>
        <xsl:variable name="baseline" select="$baseline + (if (ancestor::yes/type/integral) then $delta * ancestor::yes/type/integral/numberOfLeaves else 0)"/>-->
                ...
                <xsl:otherwise>
                    ...
                    <path xmlns="http://www.w3.org/2000/svg">
                        ...
                        <xsl:attribute name="d">
                            ...
                            <xsl:text>,</xsl:text>
                            <xsl:value-of
                                select="$baseline - ($delta + ($delta * $countComponents) - ($delta * $currentComponent))"/>
                            <xsl:text>&#32;L</xsl:text>
                            <xsl:value-of select="$Ax + $delta + 10"/>
                            <xsl:text>,</xsl:text>
                            <xsl:value-of
                ...
                <xsl:when test="./type/hook/double/yes">
                    <!--<path xmlns="http://www.w3.org/2000/svg">
                        ...
                        <xsl:attribute name="d">
                            ...
                            <xsl:text>&#32;</xsl:text>
                            <xsl:value-of
                                select="$Ax - ($delta * $countComponents) + $delta - 1 + (count(preceding-sibling::component) * $delta) - 3"/>
                            <xsl:text>,</xsl:text>
                            <xsl:value-of select="$baseline + 10"/>
                            <!-\-<xsl:text>&#32;Q</xsl:text>
                            <xsl:value-of
    ...
    <xsl:template name="componentAttachment">
        ...
        <!-- <xsl:variable name="rotation"
            select="if (boolean((ancestor::unit/preceding-sibling::unit) eq false() and $use eq false()) or (pastedown/yes and $use eq false()) or (following-sibling::component/pastedown/yes  and $use eq false()) or (preceding-sibling::component/pastedown/yes  and $use eq false())) then 45 else 0"/>
        -->
        <xsl:variable name="rotation"
            select="if (boolean(
        ...
        <g xmlns="http://www.w3.org/2000/svg">
            <path>
                ...
                <xsl:attribute name="d">
                    ...
                    <xsl:text>,</xsl:text>
                    <xsl:value-of select="$baseline - ($delta * $countComponents) - 3"/>
                </xsl:attribute>
            </path>
        </g>
    </xsl:template>-->
                                        ...
                                        <tspan xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <tspan xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                <xsl:choose>
                                                  ...
                                                  <xsl:text>other</xsl:text>
                                                  <!--
                                                  <xsl:value-of select="type/other/text()"/>-->
                                                  </xsl:when>
                                                  </xsl:choose>
                                                  </xsl:when>
                                                  <xsl:otherwise>
